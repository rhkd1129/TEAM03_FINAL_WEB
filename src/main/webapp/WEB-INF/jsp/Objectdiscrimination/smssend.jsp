...
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core"%>
    ...
    if(action.equals("go")) {
        ...
        String user_id = base64Encode("dudow123wjs"); // SMS아이디
        String secure = base64Encode("a56b86ef3cbcacbcc1fe464b36363369");//인증키
        ...
        //전화번호 조건 검사
        if(request.getParameter("rphone").replace("-", "").length() < 8 ||	//단 하나라도 조건을 만족한다면
        		request.getParameter("rphone").replaceAll("-","").length() > 11	||
        		request.getParameter("rphone").charAt(0) != '0'){
 			out.println("<script>alert('핸드폰 번호를 바르게 입력하세요.')</script");
        }else{           
        	//핸드폰 번호가 정상일 부분에 실행!   
            //out.println(data);
...
<c:redirect url="/Objectdiscrimination/fixlayout.do"></c:redirect>